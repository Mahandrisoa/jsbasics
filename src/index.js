...
export const isPalindrome = str => {
  ...
  let firstWord = splittedString.shift();
  let lastWord = splittedString.pop();
  return firstWord.toUpperCase() === reverseString(lastWord).toUpperCase();
};
...
export const nestedSum = arr => {
  const flatDeep = toFlat => {
    return toFlat.reduce(
      (acc, val) =>
        acc.concat(
          Array.isArray(val) ? flatDeep(val) : typeof val === 'number' ? val : 0
        ),
      []
    );
  };
  return flatDeep(arr).reduce((a, b) => a + b);
};
...
export const retireDe = (tab, ...elms) => {
  let setElms = new Set(elms);
  setElms.forEach(val => {
    let idx = tab.indexOf(val);
    while (idx != -1) {
      tab.splice(idx, 1);
      idx = tab.indexOf(val, idx + 1);
    }
  });
  return tab;
};
...
//       (acc, val, index) =>
//         Array.isArray(val)
//           ? acc.concat(flatDeep(val.filter(x => x !== null)))
//           : [...acc, val],
//       []
//     );
//   };
...
export const enumerer = (tab, separateur = ', ', fin = separateur) => {
  return tab.join(separateur).replace(/, ([^,]*)$/, ` ${fin} $1`);
};
...
export const nMax = (tab, n = 1) => {
  const numbers = tab
    .reduce((acc, val) => (typeof val === 'number' ? [...acc, val] : []), [])
    .sort((a, b) => b - a); // descending sort

  if (n < tab.length) {
    return numbers.splice(0, n);
  } else {
    // if n > tab.length or equal to 0
    return numbers;
  }
};
