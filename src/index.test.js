...
describe('JS Basics Tests', () => {
  ...
  describe('Test echantillon', () => {
    ...
    const result = echantillon(monTab);

    test('Should return an defined element', () => {
      expect(monTab.includes(result)).toBeDefined();
    });

    test('Should return undefined', () => {
      expect(echantillon([])).toBeUndefined();
    });

    test('Should return an element contained in the array', () => {
      expect(monTab).toContain(result);
    });
  });
  ...
  describe('Test permutations', () => {
    ...
    test('tableau avec un élèment', () => {
      expect(permutations([6]).length).toEqual(1);
      expect(permutations([6])).toEqual([6]);
    });

    test('nombre de permutations possible', () => {
      expect(permutations([1, 3])).toHaveLength(2);
    });

    test('tableau avec 2 élèments', () => {
      const expected = [
        [1, 3],
        [3, 1],
      ];
      expect(permutations([1, 3])).toEqual(expect.arrayContaining(expected));
    });
    test('tableau avec plusieurs élèments', () => {
      const expected = [
        [0, 1, 2],
        [0, 2, 1],
        [1, 0, 2],
      ...
      expect(permutations([0, 1, 2])).toEqual(expect.arrayContaining(expected));
      // if is in the same order
      expect(permutations([0, 1, 2])).toEqual(expected);
    });
    test('existance de doublons', () => {
      const expected = [
        [0, 1, 1],
        [0, 1, 1],
        [1, 0, 1],
      ...
      expect(permutations([0, 1, 1])).toEqual(expect.arrayContaining(expected));
    });
  });
  ...
  describe('Test echantillon', () => {
    ...
    const arr = [1, 2, false, []];
    beforeEach(() => {
      // spy on random function
      spy = jest.spyOn(Math, 'random');
    });

    test('spied function has been invoked and result is equal', () => {
      // la succession de ces appels est important pour prendre la valeur
      // spied
      const result = echantillon(arr);
      const idx = Math.floor(spy.mock.results[0].value * arr.length);
      expect(spy).toHaveBeenCalled();
      expect(result).toEqual(arr[idx]);
    });

    test('echantillon sur tableau à un élèment', () => {
      expect(echantillon([1])).toEqual(1);
    });

    test('echantillon sur tableau vide', () => {
      expect(echantillon([])).toBeUndefined();
    });
  });
  ...
  describe('Test enumerer', () => {
    ...
    test('tableau à 2 valeurs', () => {
      expect(enumerer(['Tom', 'Jerry'], ', ', 'et')).toEqual('Tom et Jerry');
    });

    test('tableau à plusieurs valeurs', () => {
      expect(enumerer(['Rouen', 'Le Havre', 'Caen'], ', ', 'et')).toEqual(
  ...
  describe('Test nMax', () => {
    ...
    test('retourne les 2-Max values', () => {
      expect(nMax([1, 2, 3], 2)).toEqual([3, 2]);
    });

    test('retourne les n-Max values', () => {
      expect(nMax([1, 2, 3], 20)).toEqual([3, 2, 1]);
    });

    describe('ne marche que sur les tableau de Number', () => {
      test('plus grand nombre depuis le multi-type', () => {
        expect(nMax(['1', '2', 3], 2)).toEqual([3]);
      });

      test("n'agit que sur les tableaux de number", () => {
